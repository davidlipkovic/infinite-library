...
function addHamburgerLanguages(){
    ...
    document.getElementById("kazakhHamburger").onclick = function(){onHamburgerClick('kz')};
}
        ...
        if(!cameraMode){
            ...
            Html5Qrcode.getCameras().then(devices => {
                ...
                if (devices && devices.length) {
                    ...
                    const ultraWideRegex = /(ultra[ -]?wide)/i;
                    ...
                    function findCameraByPriority(cameraList, patterns) {
                        for (const pattern of patterns) {
                            const foundCamera = cameraList.find(camera => pattern.test(camera));
                            if (foundCamera) {
                                return foundCamera; // Return the first match found
                            }
                        }
                        return undefined; // In case no matches are found
                    }

                    const selectedCamera = findCameraByPriority(cameras, regexPatterns);
